...
export default function FindEmployees({ dic }: { dic: NewOrderDictionary }) {
 ...
 const { data, hasNextPage, fetchNextPage, isFetching, refetch, isSuccess } =
  useInfiniteQuery({
   ...
   queryKey: [newOrderKey, 'employees', debouncedSearch || 'all'],
   initialPageParam: {
    limit: 300,
    offset: 1,
   },
   async queryFn({ signal, pageParam }) {
    const res = await getPagedEmployee({
     signal,
 ...
 return (
  <DrawerContent className='h-[min(80svh,35rem)]'>
   <DrawerHeader className='hidden'>
    <DrawerTitle>{dic.orderInfo.employee}</DrawerTitle>
   </DrawerHeader>
   <div className='p-4 pt-2 pb-4 border-b border-input flex flex-wrap justify-between gap-4'>
    <h1 className='text-xl font-medium text-neutral-600 dark:text-neutral-400'>
     {dic.orderInfo.employee}{' '}
     <span className='text-lg'>
      ({dic.findEmployees.result}: {data?.pages[0].rowsCount})
     </span>
    </h1>
     ...
     {isSuccess && !data.pages.length ? (
      ...
      <ul className='grid gap-2 grid-cols-[repeat(auto-fill,minmax(14rem,15rem))]'>
       {data?.pages.map((group, i) => (
        <Fragment key={i}>
         {group.rows.map(({ id, code, personFirstName, personLastName }) => (
          <li key={id}>
           <DrawerClose asChild>
            <Button
             variant={'outline'}
            ...
            >
             <div className='grid gap-1'>
              ...
              <div>
               <Highlighter
                searchWords={[debouncedSearch]}
                textToHighlight={personFirstName + ' ' + personLastName}
               />
              </div>
             </div>
            </Button>
