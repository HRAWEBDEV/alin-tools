...
export default function NewOrderHeader({ dic }: { dic: NewOrderDictionary }) {
 ...
 const orderRedirectLink = tableValue
  ? (`/${locale}/${routeDepartment.id}/restaurant/${routeProgram.id}/new-order?salonID=${salonID}&salonName=${salonName}&tableID=${Number(tableValue.key)}&tableNo=${Number(tableValue.value)}&fromSalons=true` as const)
  : '';
