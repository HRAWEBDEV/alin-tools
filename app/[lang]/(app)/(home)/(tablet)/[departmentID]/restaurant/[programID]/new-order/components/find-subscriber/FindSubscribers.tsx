...
import { newOrderKey, getSubscribers } from '../../services/newOrderApiActions';
...
export default function FindSubscriber({ dic }: { dic: NewOrderDictionary }) {
 ...
 const { data, hasNextPage, fetchNextPage, isFetching, refetch, isSuccess } =
  useInfiniteQuery({
   ...
   queryKey: [newOrderKey, routeProgram.id, 'subscribers', debouncedSearch],
   initialPageParam: {
    limit: 300,
    offset: 1,
   },
     ...
     {isSuccess && !data.pages.length ? (
      ...
      <ul className='grid gap-2 grid-cols-[repeat(auto-fill,minmax(14rem,15rem))]'>
       {data?.pages.map((group, i) => (
        <Fragment key={i}>
         {group.rows.map(({ personID, code, nationalCode, mobileNo, name }) => (
          <li key={personID}>
           <DrawerClose asChild>
            <Button
             variant={'outline'}
