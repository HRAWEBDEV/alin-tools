...
export default function SalonBaseConfigProvider({
 ...
 const { routeOwner, routeDepartment, routeProgram } = useUserInfoRouter();
 ...
 }, [initDataSuccess, routeDepartment, routeOwner, routeProgram]);
 useEffect(() => {
  getSalonTables();
 }, [getSalonTables]);
 ...
 useEffect(() => {
  ...
  router.replace(
   `/${locale}/${routeDepartment.id}/restaurant/${routeProgram.id}/salons?${newSearchParams.toString()}`,
  );
 }, [
  selectedHall,
  locale,
