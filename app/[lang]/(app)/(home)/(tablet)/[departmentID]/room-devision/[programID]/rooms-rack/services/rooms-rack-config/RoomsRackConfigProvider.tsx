...
export function RoomsRackConfigProvider({
  ...
  const [showGuestExpenses, setShowGuestExpenses] = useState(false);
  ...
  useEffect(() => {
    ...
    rackReportConnection.on('NotifChanged', () => {
      ...
      if (!notificationAudioRef.current) {
        notificationAudioRef.current = new Audio('/sounds/notification-sound.mp3')
      }
  ...
  }, [routeDepartment, routeOwner, routeProgram]);
  //
  ...
  useEffect(() => {
    ...
    router.replace(
      `/${locale}/${routeDepartment.id}/room-devision/${routeProgram.id}/rooms-rack?${newSearchParams.toString()}`,
    );
  }, [
    locale,
    router,
    ...
    <rackConfigContext.Provider value={ctx}>
      <FormProvider {...rackFiltersUseForm}>
        ...
        <RoomMenu
          ...
          showRoomNotes={showRoomNotes}
          setShowRoomNotes={setShowRoomNotes}
          showOutOfOrder={showOutOfOrder}
          setShowOutOfOrder={setShowOutOfOrder}
          showGuestExpenses={showGuestExpenses}
          setShowGuestExpenses={setShowGuestExpenses}
          setIsOpen={(value) => {
            if (value) {
              return;
            } else {
