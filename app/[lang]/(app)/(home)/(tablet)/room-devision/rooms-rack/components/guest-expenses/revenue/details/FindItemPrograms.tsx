...
export default function FindItemPrograms({
 dic,
 programID,
 onChangeItem,
}: {
 ...
 programID: string;
 onChangeItem: (item: ItemProgram) => unknown;
}) {
 ...
 const { data, hasNextPage, fetchNextPage, isFetching, refetch, isSuccess } =
  useInfiniteQuery({
   queryKey: [getItemProgramsApi, programID, debouncedSearch],
   initialPageParam: {
    limit: 100,
    offset: 1,
   },
   ...
   <div className='p-4 pt-2 pb-4 border-b border-input flex flex-wrap justify-between gap-4'>
    ...
    <div className='w-[20rem] grid grid-cols-[1fr_max-content] gap-2'>
     <InputGroup>
      <InputGroupInput
       type='search'
       placeholder={dic.invoiceDetails.search}
       value={searchedText}
